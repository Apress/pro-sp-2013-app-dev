...
#Region "public methods"
    ...
    ''' </summary>
    Public Shared Sub TrustAllCertificates()
    ...
    ''' exception is thrown. If the token is valid, a JsonWebSecurityToken based on the context token is returned.
    ''' </summary>
    ''' <param name="contextTokenString">The context token to validate</param>
    ''' <param name="appHostName">The URL authority, consisting of  Domain Name System (DNS) host name or IP address and the port number, to use for token audience validation.
    ''' If Nothing, HostedAppHostName web.config setting is used instead.</param>
    ...
    Public Shared Function ReadAndValidateContextToken(contextTokenString As String, Optional appHostName As String = Nothing) As SharePointContextToken
        ...
        If appHostName Is Nothing Then
            appHostName = HostedAppHostName
        End If

        Dim definedRealm As String = If(Realm, token.Realm)
        Dim principal As String = GetFormattedPrincipal(ClientId, appHostName, definedRealm)
        If Not StringComparer.OrdinalIgnoreCase.Equals(token.Audience, principal) Then
            Throw New Microsoft.IdentityModel.Tokens.AudienceUriValidationFailedException([String].Format(CultureInfo.CurrentCulture, """{0}"" is not the intended audience ""{1}""", principal, token.Audience))
        End If
    ...
    Public Shared Function GetAccessToken(contextToken As SharePointContextToken, targetHost As String) As OAuth2AccessTokenResponse
        ...
        Dim definedRealm As String = If(Realm, contextToken.Realm)

        Dim resource As String = GetFormattedPrincipal(targetPrincipalName, targetHost, definedRealm)
        Dim formattedClientId As String = GetFormattedPrincipal(ClientId, Nothing, definedRealm)
        ...
        Try
            oauth2Response = TryCast(client.Issue(AcsMetadataParser.GetStsUrl(definedRealm), oauth2Request), OAuth2AccessTokenResponse)
    ...
    ''' <param name="targetRealm">Realm to use for the access token's nameid and audience</param>
    ''' <returns>An access token with an audience of the target principal</returns>
    Public Function GetAccessToken(refreshToken As String, targetPrincipalName As String, targetHost As String, targetRealm As String) As OAuth2AccessTokenResponse
    ...
    ''' <summary>
    ''' Retrieves an S2S access token signed by the application's private certificate and intended for the 
    ''' specified targetHost. If specified realm is Nothing, the "Realm" setting in web.config will be used instead.
    ''' </summary>
    ''' <param name="targetPrincipalName">Name of the target principal to retrieve an access token for</param>
    ''' <param name="targetHost">Url authority of the target principal</param>
    ''' <param name="targetRealm">Realm to use for the access token's nameid and audience</param>
    ''' <returns>An access token with an audience of the target principal</returns>
    Public Shared Function GetS2SAccessToken(targetPrincipalName As String, targetHost As String, targetRealm As String) As OAuth2AccessTokenResponse
    ...
    Public Shared Function CreateRemoteEventReceiverClientContext(properties As SPRemoteEventProperties) As ClientContext
        Dim sharepointUrl As Uri = Nothing
        ...
        If ClientCertificate IsNot Nothing Then
            Return TokenHelper.GetS2SClientContextWithWindowsIdentity(sharepointUrl, Nothing)
        Else
...
#Region "private fields"
    ...
    Private Const SHAREPOINT_PID As String = "00000003-0000-0ff1-ce00-000000000000"

    Private Const AccessTokenParameterName As String = "AccessToken"
    ...
    Private Const DelegationService As String = "DelegationService1.0"
    Private Const AcsManagementServiceRelativeUrl As String = "r4/mgmt/service/"
    Private Const AcsManagementServiceAppliesTo As String = "v2/mgmt/service/"
    Private Const AcsOAuthRelativeUrl As String = "v2/OAuth2-13"
    Private Const S2SProtocol As String = "OAuth2"
    Private Const DelegationIssuance As String = "DelegationIssuance1.0"
    Private Const DelegationIssuanceRequestTokenUrlSuffix As String = "RequestAccessToken"
    Private Const NameIdentifierClaimType As String = JsonWebTokenConstants.ReservedClaims.NameIdentifier
    Private Const IdentityProviderClaimType As String = JsonWebTokenConstants.ReservedClaims.IdentityProvider
    Private Const TrustedForImpersonationClaimType As String = "trustedfordelegation"
    Private Const ApplicationContextClaimType As String = JsonWebTokenConstants.ReservedClaims.AppContext
    Private Const ActorTokenClaimType As String = JsonWebTokenConstants.ReservedClaims.ActorToken
    Private Const SmtpClaimType As String = "smtp"
    Private Const SipClaimType As String = "sip"
    Private Const TokenLifetimeMinutes As Integer = 1000000
    ...
    Private Shared ClientCertificate As X509Certificate2 = If((String.IsNullOrEmpty(ClientSigningCertificatePath) OrElse String.IsNullOrEmpty(ClientSigningCertificatePassword)), Nothing, New X509Certificate2(ClientSigningCertificatePath, ClientSigningCertificatePassword))
    Private Shared SigningCredentials As Microsoft.IdentityModel.SecurityTokenService.X509SigningCredentials = If((ClientCertificate Is Nothing), Nothing, New Microsoft.IdentityModel.SecurityTokenService.X509SigningCredentials(ClientCertificate, SecurityAlgorithms.RsaSha256Signature, SecurityAlgorithms.Sha256Digest))
...
#Region "private methods"
    ...
    Private Shared Function CreateAcsClientContextForUrl(properties As SPRemoteEventProperties, sharepointUrl As Uri) As ClientContext
        ...
        Dim requestProperty As HttpRequestMessageProperty = DirectCast(OperationContext.Current.IncomingMessageProperties(HttpRequestMessageProperty.Name), HttpRequestMessageProperty)
        Dim contextToken As SharePointContextToken = TokenHelper.ReadAndValidateContextToken(contextTokenString, requestProperty.Headers(HttpRequestHeader.Host))

        Dim accessToken As String = TokenHelper.GetAccessToken(contextToken, sharepointUrl.Authority).AccessToken
    ...
    Private Shared Function GetFormattedPrincipal(principalName As String, hostName As String, realm As String) As String
    ...
    Private Shared Function GetAcsPrincipalName(realm As String) As String
    ...
    Private Shared Function GetRealmFromTargetUrl(targetApplicationUri As Uri) As String
        ...
        Catch e As WebException
            ...
            Dim realm As String = bearerResponseHeader.Substring(bearerResponseHeader.IndexOf("Bearer realm=""") + 14, 36)

            Dim realmGuid As Guid

            If Guid.TryParse(realm, realmGuid) Then
                Return realm
    ...
    Private Shared Function GetS2SAccessTokenWithClaims(targetApplicationHostName As String, targetRealm As String, claims As IEnumerable(Of JsonWebTokenClaim)) As String
        Return IssueToken(ClientId, IssuerId, targetRealm, SHAREPOINT_PID, targetRealm, targetApplicationHostName, True, _
    ...
    Private Shared Function GetClaimsWithWindowsIdentity(identity As WindowsIdentity) As JsonWebTokenClaim()
        Dim claims As JsonWebTokenClaim() = New JsonWebTokenClaim() {New JsonWebTokenClaim(TokenHelper.NameIdentifierClaimType, identity.User.Value.ToLower()), New JsonWebTokenClaim("nii", "urn:office:idp:activedirectory")}
        Return claims
    End Function
    ...
    Private Shared Function IssueToken(sourceApplication As String, issuerApplication As String, sourceRealm As String, targetApplication As String, targetRealm As String, targetApplicationHostName As String, trustedForDelegation As Boolean, _
        ...
        ' Create token
        Dim actorToken As New JsonWebSecurityToken(issuer:=issuer, audience:=audience, validFrom:=DateTime.UtcNow, validTo:=DateTime.UtcNow.AddMinutes(TokenLifetimeMinutes), signingCredentials:=SigningCredentials, claims:=actorClaims)
        ...
        Dim jsonToken As New JsonWebSecurityToken(nameid, audience, DateTime.UtcNow, DateTime.UtcNow.AddMinutes(10), outerClaims)
    ...
    Private Shared Function EnsureTrailingSlash(url As String) As String
...
Public Class SharePointContextToken
    ...
    Public ReadOnly Property SecurityTokenServiceUri() As String
        Get
            ...
            Dim ctx As New ClientContext("http://tempuri.org")
            Dim dict As Dictionary(Of String, Object) = DirectCast(ctx.ParseObjectFromJsonString(appctx), Dictionary(Of String, Object))
            Dim cacheKey As String = DirectCast(dict("SecurityTokenServiceUri"), String)
...
Public Class MultipleSymmetricKeySecurityToken
    ...
    ''' <param name="keys">An enumeration of Byte arrays that contain the symmetric keys.</param>
    Public Sub New(id As String, keys As IEnumerable(Of Byte()))
        ...
        Me.m_id = id
        Me.m_effectiveTime = DateTime.UtcNow
        Me.m_securityKeys = CreateSymmetricSecurityKeys(keys)
    End Sub
    ...
    Public Overrides ReadOnly Property SecurityKeys() As ReadOnlyCollection(Of SecurityKey)
        Get
            Return Me.m_securityKeys.AsReadOnly()
        End Get
    End Property
