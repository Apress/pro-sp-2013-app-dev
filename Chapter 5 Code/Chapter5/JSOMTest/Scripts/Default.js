...
function defaultViewModel() {
    ...
    self.startLoad = function () {
        ...
        self.context.load(self.web); // adds a "load" request for the web object to the next batch

        self.user = self.web.get_currentUser();
        self.context.load(self.user); // adds a "load" request for the user object to the next batch
